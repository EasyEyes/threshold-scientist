...
describe("Prolific study creation", () => {
  ...
  it("creates a Prolific study for a retrieved study with empty recruitment metadata", async () => {
    const {
      createProlificStudyIdFile,
      generateAndUploadCompletionURL,
      getProlificStudyId,
    ...
    expect(prolificCreateDraft).toHaveBeenCalledWith(
      expect.objectContaining({
        currentExperiment: expect.objectContaining({
          experimentUrl:
    ...
    expect(createProlificStudyIdFile).toHaveBeenCalledWith(
      expect.objectContaining({ id: 42 }),
      expect.anything(),
      "new-study-id",
    );
    expect(prolificTab.close).not.toHaveBeenCalled();
