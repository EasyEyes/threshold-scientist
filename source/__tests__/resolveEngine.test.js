...
const manifest = (releaseId, version) => ({
  schemaVersion: 1,
  releaseId,
  contractVersion: 1,
  engine: {
...
describe("resolveEngine — resolving an explicit release id", () => {
  it("looks up the release's engineVersion and imports exactly that engine release's URL", async () => {
    ...
    const manifestClient = fakeManifestClient({
      entries: {
        "2026-03-01": manifest("2026-03-01", "2026.3.1"),
      },
    });
    ...
    expect(importModule).toHaveBeenCalledWith(
      "https://cdn.jsdelivr.net/npm/@easyeyes/threshold-engine@2026.3.1/dist/index.js",
    );
  ...
  it("exposes the release's manifest-pinned glossary and phrases versions, for the compile flow to fetch by (issue #182)", async () => {
    ...
    const manifestClient = fakeManifestClient({
      entries: {
        "2026-03-01": {
          ...manifest("2026-03-01", "2026.3.1"),
          glossary: { version: "4.2", digest: "sha256-test" },
          phrases: { version: "2.1", digest: "sha256-test" },
        },
      },
    });
...
describe('resolveEngine — resolving "latest"', () => {
  it("resolves the latest pointer, then loads that release's manifest entry", async () => {
    ...
    const manifestClient = fakeManifestClient({
      latest: "2026-06-01",
      entries: {
        "2026-06-01": manifest("2026-06-01", "2026.6.1"),
      },
    });
    ...
    expect(importModule).toHaveBeenCalledWith(
      "https://cdn.jsdelivr.net/npm/@easyeyes/threshold-engine@2026.6.1/dist/index.js",
    );
